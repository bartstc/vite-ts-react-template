...
## Sharing test fixtures between unit and E2E tests

**Recommendation: Directly import domain fixtures from `@/test-lib` in your E2E tests.** Your project already uses `@/*` path mapping and E2E tests already import from `@/lib`, so there's no architectural reason to create a separate shared directory.

### Why share fixtures across test levels?

- **Single source of truth** — Test data defined once, eliminates duplication
- **Type safety** — Shared TypeScript types guarantee consistency across all test levels
- **Maintenance** — Change fixture once, all tests (unit, component, E2E) updated automatically
- **Domain alignment** — E2E tests use the same entities and business rules as unit tests
- **Refactoring safety** — Type errors surface immediately across the entire test suite

### The createFixture pattern in your codebase

Your `src/test-lib/fixtures/createFixture.ts` provides three key capabilities:

```typescript
export function createFixture<T>(template: T) {
  return {
    toStructure: () => clone(template),              // Deep clone of base template
    createPermutation: (extend?: DeepPartial<T>) => T,  // Single variation with overrides
    createCollection: (extend: DeepPartial<T>[]) => T[], // Multiple variations
  };
}
```

This pattern ensures immutability, supports partial overrides with `DeepPartial<T>`, and enables efficient collection generation.

### Import strategy for E2E tests
...
- Consistent import style across the project (E2E already imports from `@/lib`)
- No duplication or synchronization issues
- Type safety maintained seamlessly
- Changes to fixtures propagate immediately to all tests

### When to create E2E-specific fixtures

Create **E2E-specific fixture extensions** only for concerns that don't belong in domain models:
...
Keep these in `e2e/fixtures/` to maintain clear separation between domain data and test infrastructure.

### What NOT to share

Some E2E-specific data should remain local to E2E tests:

- **Playwright configuration** — Use native Playwright config, not fixtures
- **API mocking responses** — E2E tests should hit real APIs (unlike unit tests with MSW)
- **Test infrastructure constants** — Animation delays, debounce timers, etc.

### Integration with Playwright fixtures

Combine domain fixtures with Playwright's fixture system:
...
type Fixtures = {
  testUser: IUser; // ← Domain fixture
  signInPage: SignInPage; // ← Page object fixture
};

export const test = base.extend<Fixtures>({
  testUser: async ({}, use) => {
    await use(UserFixture.createPermutation({ username: "testuser" }));
  },
  signInPage: async ({ page }, use) => {
    await use(new SignInPage(page));
  },
...
### Folder structure with shared fixtures

```
project-root/
├── src/
│   └── test-lib/
│       └── fixtures/          # ← Shared domain fixtures (unit + E2E)
│           ├── UserFixture.ts
│           ├── ProductFixture.ts
│           └── CartFixture.ts
└── e2e/
    ├── fixtures/              # ← E2E-specific fixtures only
    │   ├── pageFixtures.ts    # Playwright fixture extensions
    │   └── credentialFixtures.ts  # Auth credentials
    └── pages/                 # ← Page objects
```

**Key principle:** Domain data lives in `@/test-lib/fixtures`, test infrastructure lives in `e2e/fixtures/`.
